    ...
    public class Soft2DManagerUI : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (Soft2DManager.enableForceField)
                EditorGUILayout.PropertyField(forceFieldScale,
                    new GUIContent("Force Field Scale", "Force field scale"));
            EditorGUILayout.PropertyField(enableWorldBoundary,
                new GUIContent("Enable World Boundary", "Enable world boundary"));
            if (Soft2DManager.enableWorldBoundary)
            {
                EditorGUILayout.PropertyField(collisionType,
                    new GUIContent("Collision Type", "Boundary's collision type"));
            ...
            EditorGUILayout.PropertyField(enableDebug,
                new GUIContent("Enable Debugging Tools", "enable Debugging Tools"));
            if (Soft2DManager.enableDebuggingTools)
            {
                EditorGUILayout.PropertyField(colliderCol,
                    new GUIContent("Collider Color", "Collider's color showed on Debugging Tools"));
                EditorGUILayout.PropertyField(triggerCol,
                    new GUIContent("Trigger Color", "Trigger's color showed on Debugging Tools"));
            }
            ...
            var layer = EditorGUILayout.LayerField("Particle Sorting Layer", Soft2DManager.layerIndex);
            EditorGUILayout.Space(10);
            ...
            if (Soft2DManager.shaderType == ShaderType.Unlit)
            {
                ...
                EditorGUILayout.PropertyField(instanceSize,
                    new GUIContent("Instance Size", "Particle instance's size"));
            }
            ...
            EditorGUILayout.PropertyField(S2Substep,
                new GUIContent("SubStep", "The time step of the internal sub-step"));
            EditorGUILayout.EndVertical();

            EditorGUILayout.Space(10);
            EditorGUILayout.LabelField("Max Num Settings", EditorStyles.boldLabel);
            EditorGUILayout.BeginVertical("Box");
            EditorGUILayout.PropertyField(S2MaxParticleNum,
                new GUIContent("Max Particle Number", "The maximum allowed number of particles"));
            EditorGUILayout.PropertyField(S2MaxBodyNum,
                new GUIContent("Max Body Number", "The maximum allowed number of bodies"));
            EditorGUILayout.PropertyField(S2MaxTriggerNum,
                new GUIContent("Max Trigger Number", "The maximum allowed number of triggers"));
            EditorGUILayout.EndVertical();

            EditorGUILayout.Space(10);
            EditorGUILayout.LabelField("Resolution Settings", EditorStyles.boldLabel);
            EditorGUILayout.BeginVertical("Box");
            EditorGUILayout.PropertyField(S2GridResolution,
                new GUIContent("Grid Resolution", "Debugging Tools' gird resolution"));
            EditorGUILayout.PropertyField(S2FineGridResolution,
                new GUIContent("Fine Grid Scale",
                    "A scale factor of fine grid resolution compared to background grid resolution"));
            EditorGUILayout.EndVertical();

            EditorGUILayout.Space(10);
            EditorGUILayout.LabelField("Collision Settings", EditorStyles.boldLabel);
            EditorGUILayout.BeginVertical("Box");
            EditorGUILayout.PropertyField(S2NormalForceScale,
                new GUIContent("Normal Force Scale", "Collision penalty force scale along normal direction"));
            EditorGUILayout.PropertyField(S2VelocityForceScale,
                new GUIContent("Velocity Force Scale", "Collision penalty force scale along velocity direction"));
            ...
            EditorGUILayout.PropertyField(S2EnableWorldQuery,
                new GUIContent("Enable World Query", "Whether enable world query"));
        ...
        internal static Soft2DManager Soft2DManager;
