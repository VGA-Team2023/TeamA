...
public class CriAtomEx3dListener : CriDisposable
{
	 ...
	 * <para header='注意'>本関数はAtomExプレーヤのパラメータ更新（ CriWare.CriAtomExPlayer::UpdateAll, CriWare.CriAtomExPlayer::Update ）
	 * とは独立して動作します。<br/>
	 ...
	 * <para header='注意'>クリアしたパラメータを実際に適用するには、 CriWare.CriAtomEx3dListener::Update 関数を呼び出す必要があります。</para>
	 * </remarks>
	 */
	 ...
	 * 設定したパラメータを実際に適用するには、 CriWare.CriAtomEx3dListener::Update 関数を呼び出す必要があります。</para>
	 * </remarks>
	...
	/**
	 * <summary>3Dリスナーを破棄可能かどうかのチェック</summary>
	 * <returns>3Dリスナーの状態（true = 破棄可能、false = 破棄不可）</returns>
	 * <remarks>
	 * <para header='説明'>3Dリスナーを破棄可能かどうかをチェックします。<br/>
	 * 3DリスナーをセットしたAtomExプレーヤーで再生している音声がある場合、<br/>
	 * <see cref='Dispose'/> 関数を呼び出しても、破棄に失敗します。<br/>
	 * 破棄する3Dリスナーを設定したAtomExプレーヤーをすべて破棄するか、<br/>
	 * 別の3Dリスナーを設定してください。<br/></para>
	 * </remarks>
	 * <seealso cref='Dispose'/>
	 * <seealso cref='CriAtomExPlayer.Set3dListener'/>
	 */
	public bool IsDestroyable() {
		return criAtomEx3dListener_IsDestroyable(this.handle);
	}
	...
	private static bool criAtomEx3dListener_IsDestroyable(IntPtr ex_3d_listener) { return true; }
#endif
