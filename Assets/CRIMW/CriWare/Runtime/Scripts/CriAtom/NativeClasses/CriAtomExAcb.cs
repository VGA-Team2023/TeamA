...
public class CriAtomExAcb : CriDisposable
{
	 ...
	 * （ CriWare.CriAtomExAcb ）が返されます。<br/>
	 * AtomExプレーヤに対し、 CriWare.CriAtomExPlayer::SetCue 関数でACBハンドル、
	 * および再生するキュー名を指定することで、
	...
	/**
	 * <summary>ACBデータのロード</summary>
	 * <param name='acbData'>固定されたACBデータのポインタ</param>
	 * <param name='dataSize'>ACBのデータサイズ</param>
	 * <param name='awbBinder'>AWB用バインダオブジェクト</param>
	 * <param name='awbPath'>AWBファイルのパス</param>
	 * <returns>CriAtomExAcbオブジェクト</returns>
	 * <remarks>
	 * <para header='説明'>メモリ上に配置されたACBデータをロードし、キュー再生に必要な情報を取り込みます。<br/>
	 * <br/>
	 * 第2引数の awbPath にはストリーム再生用のAWBファイルのパスを指定します。<br/>
	 * （オンメモリ再生のみのACBデータをロードする場合、
	 * awbPath にセットした値は無視されます。）<br/>
	 * <br/>
	 * AWBファイルをCPKファイルにパッキングしている場合、
	 ...
	 * ACBファイルのロードは、シーンの切り替わり等、負荷変動を許容できる
	 * タイミングで行うようお願いいたします。<br/></para>
	 * <para header='備考'>本メソッドはネイティブメモリ領域のデータを指定する際に利用することを想定しており、<br/>
	 * <paramref name='acbData'/>にはメモリ上で固定されたデータを示すポインタを指定する必要があります。<br/>
	 * マネージド領域のバイト配列を利用する場合はCriAtomExAcb.LoadAcbData(byte[], CriFsBinder, string)をご利用ください。<br/></para>
	 * </remarks>
	 * <seealso cref='CriAtomExPlayer::SetCue'/>
	 */
	...
	/**
	 * <summary>すべてのキュー情報の取得</summary>
	 * <returns>キュー情報配列</returns>
	 * <remarks>
	 * <para header='説明'>Acbに含まれるすべてのキュー情報を取得します。<br/></para>
	 * </remarks>
	 * <seealso cref='CriAtomExAcb::GetCueInfo'/>
	 */
	 ...
	 * CriWare.CriAtomExAcb::Dispose 関数内で長時間処理がブロックされる可能性があります。）<br/></para>
	 * </remarks>
	 */
...
public class CriAtomExAcbLoader : CriDisposable
{
	 ...
	 * 戻り値に対して CriWare.CriAtomExAcbLoader::GetStatus を呼び出してロード状態を確認し、
	 * 状態が Complete に遷移したら、 CriWare.CriAtomExAcbLoader::MoveAcb により CriWare.CriAtomExAcb オブジェクトが取得可能です。<br/></para>
	 * </remarks>
	 */
	 ...
	 * 戻り値に対して CriWare.CriAtomExAcbLoader::GetStatus を呼び出してロード状態を確認し、
	 * 状態が Complete に遷移したら、 CriWare.CriAtomExAcbLoader::MoveAcb により CriWare.CriAtomExAcb が取得可能です。<br/></para>
	 * </remarks>
	 */
	 ...
	 * <para header='備考'>本メソッドはネイティブメモリ領域のデータを指定する際に利用することを想定しており、<br/>
	 * <paramref name='acbData'/>にはメモリ上で固定されたデータを示すポインタを指定する必要があります。<br/>
	 * マネージド領域のバイト配列を利用する場合はCriAtomExAcbLoader.LoadAcbDataAsync(byte[], CriFsBinder, string, bool)をご利用ください。<br/></para>
	 * </remarks>
	 */
	 ...
	 * 本関数の戻り値が Complete に遷移したら、 CriWare.CriAtomExAcbLoader::MoveAcb により CriWare.CriAtomExAcb の取得が可能です。<br/></para>
	 * </remarks>
	 */
	 ...
	 * 本関数は、 CriWare.CriAtomExAcbLoader::GetStatus の戻り値が Complete に遷移してから呼び出してください。<br/></para>
	 * </remarks>
	 */
