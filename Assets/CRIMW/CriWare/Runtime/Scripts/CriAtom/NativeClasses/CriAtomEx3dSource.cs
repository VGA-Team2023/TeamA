...
public class CriAtomEx3dSource : CriDisposable
{
	 ...
	 * <para header='注意'>本関数はAtomExプレーヤのパラメータ更新（ CriWare.CriAtomExPlayer::UpdateAll, CriAtomExPlayer::Update ）
	 * とは独立して動作します。<br/>
	 ...
	 * <para header='注意'>クリアしたパラメータを実際に適用するには、 CriWare.CriAtomEx3dSource::Update 関数を呼び出す必要があります。</para>
	 * </remarks>
	 ...
	 * <para header='注意'>設定したパラメータを実際に適用するには、 CriWare.CriAtomEx3dSource::Update 関数を呼び出す必要があります。</para>
	 * </remarks>
	 * <seealso cref='CriAtomEx3dSource::Update'/>
	 */
	 ...
	 * （ CriWare.CriAtomEx3dListener::SetDistanceFactor 関数で設定）で決まります。
	 * デフォルト値は(0.0f, 0.0f, 0.0f)です。<br/>
	 ...
	 * <para header='注意'>設定したパラメーターを実際に適用するには、CriWare.CriAtomEx3dSource::Update 関数を呼び出す必要があります。<br/>
	 ...
	 * CriWare.CriAtomEx3dSource::Update 関数を呼び出す必要があります。<br/>
	 ...
	 * <para header='備考'>位置座標の算出方法として CriWare.CriAtomEx::Randomize3dCalcType::List<br/>
	 * を設定している場合のみ、本パラメーターを参照します。<br/>
	 * その他の算出方法を設定している場合、本パラメーターは無視されます。<br/></para>
	 * <para header='注意'>設定したパラメーターを実際に適用するには、CriWare.CriAtomEx3dSource::Update 関数を呼び出す必要があります。<br/>
	 * 本関数に設定した領域は、 CriWare.CriAtomEx3dSource::SetRandomPositionList 関数<br/>
	 * の実行完了後は、内部からは参照されません。<br/>
	 * 代わりに内部で確保した位置座標リストに保存します。<br/>
	 * そのため、長さが CriWare.CriAtomEx3dSource::Config::randomPositionListMaxLength を<br/>
	 ...
	 * 設定したパラメータを実際に適用するには、 CriWare.CriAtomEx3dSource::Update 関数を呼び出す必要があります。</para>
	 * </remarks>
	 ...
	 * データ側に設定されている距離AISACコントロールIDは、本関数によって上書き適用されます。<br/></para>
	 * <para header='注意'>設定したパラメータを実際に適用するには、 CriAtomEx3dSource.Update 関数を呼び出す必要があります。<br/>
	 * 本関数を実行すると指定した3D音源に前に設定されていたAISACコントールIDの値の変更によるパラメーター変更は無効化されますが、<br/>
	 * 再生済みの音声については前に適用されたパラメーターが適用され続けます。<br/>
	 * 指定する3D音源に紐づく音声が再生されていない状態で本関数を実行することを推奨します。<br/></para>
	 * </remarks>
	 * <seealso cref='CriAtomEx3dSource::Update'/>
	 */
	...
	/**
	 * <summary>3D音源を破棄可能かどうかのチェック</summary>
	 * <returns>3D音源の状態（true = 破棄可能、false = 破棄不可）</returns>
	 * <remarks>
	 * <para header='説明'>3D音源を破棄可能かどうかをチェックします。<br/>
	 * 3D音源をセットしたAtomExプレーヤーで再生している音声がある場合、<br/>
	 * <see cref='Dispose'/> 関数を呼び出しても、破棄に失敗します。<br/>
	 * 破棄する3D音源を設定したAtomExプレーヤーをすべて破棄するか、<br/>
	 * 別の3D音源を設定してください。<br/></para>
	 * </remarks>
	 * <seealso cref='Dispose'/>
	 * <seealso cref='CriAtomExPlayer.Set3dSource'/>
	 */
	public bool IsDestroyable() {
		return criAtomEx3dSource_IsDestroyable(this.handle);
	}

	/**
	 * <summary>3D音源の位置の取得</summary>
	 * <remarks>
	 * <para header='説明'>3D音源の位置を取得します。<br/>
	 * 位置は、3次元ベクトルで取得します。<br/></para>
	 * </remarks>
	 */
	public CriAtomEx.NativeVector GetPosition() {
		return criAtomEx3dSource_GetPosition(this.handle);
	}
	...
	private static bool criAtomEx3dSource_IsDestroyable(IntPtr ex_3d_source) { return true; }
	private static CriAtomEx.NativeVector criAtomEx3dSource_GetPosition(IntPtr ex_3d_source) { return new CriAtomEx.NativeVector(); }
#endif
